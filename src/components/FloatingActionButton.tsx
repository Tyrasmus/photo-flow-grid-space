...
const FloatingActionButton = ({
  ...
  const twinkleIntervalRef = useRef<NodeJS.Timeout | null>(null);

  // Generate random sparkle position
  const generateRandomSparkle = (id: number): SparkleState => {
    const positions = [
      // Close positions
      { top: '-8px', right: '-8px' },
      { top: '-12px', left: '-4px' },
      ...
      { bottom: '30%', right: '-28px' },
    ];

    const position = positions[Math.floor(Math.random() * positions.length)];
    const sizes = [6, 8, 10, 12, 14];
    const delays = ['25ms', '40ms', '60ms', '80ms', '100ms'];
  ...
  const initializeSparkles = () => {
    const newSparkles = Array.from({ length: 5 }, (_, i) => generateRandomSparkle(i));
    setSparkles(newSparkles);
  };

  // Twinkle effect - fade out and reposition sparkles
  const twinkleSparkles = () => {
    setSparkles(prev => prev.map(sparkle => ({ ...sparkle, visible: false })));
    
    setTimeout(() => {
      setSparkles(prev => prev.map(sparkle => ({ 
        ...generateRandomSparkle(sparkle.id), 
        visible: true 
      })));
    }, 300); // Wait for fade out before repositioning
  };
  ...
  const handleMouseEnter = () => {
    ...
    // Start twinkling effect every 1.5 seconds
    twinkleIntervalRef.current = setInterval(twinkleSparkles, 1500);
  };
  ...
  const handleMouseLeave = () => {
    ...
    // Clear twinkling interval
    if (twinkleIntervalRef.current) {
      clearInterval(twinkleIntervalRef.current);
      twinkleIntervalRef.current = null;
    }
  };
          ...
          {sparkles.map((sparkle) => (
            <Sparkle 
              ...
              className={`absolute text-white transition-all duration-300 ${
                isHovered && sparkle.visible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
              }`}
              style={{
                top: sparkle.top,
                right: sparkle.right,
